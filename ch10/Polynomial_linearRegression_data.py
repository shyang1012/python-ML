...
if __name__ == '__main__':
    ...
    plt.savefig(get_base_dir('images')+'/Polynomial_linearRegression_data_compare.png', dpi = 300)
    plt.close()

    # 지수함수에 자연로그를 취하면 직선이 되기 때문에 로그 변환 후의 값 테스트
    #  특성을 변환합니다
    X_log = np.log(X)
    y_sqrt = np.sqrt(y)
    ...
    plt.tight_layout()
    plt.savefig(get_base_dir('images')+'/Polynomial_linearRegression_data_apply_log.png', dpi = 300)
    plt.show()
    plt.close()