...
if __name__ == '__main__':
    ...
    print("정확도 %.3f" % knn.score(X_test_std, y_test))
    ...
    cmmnUtils.plot_decision_regions_with_test_marker(X=X_combined_std, y= y_combined_std, classifier=knn, test_idx=range(105, 150))
    plt.xlabel('petal length [standardized]')
    plt.ylabel('petal width [standardized]')
    plt.legend(loc='upper left')

    plt.tight_layout()
    plt.savefig(get_base_dir('images')+'/iris_decision_regisons_KNN.png', dpi = 300)
    plt.show()
    plt.close()
