...
if __name__ == '__main__':
    ...
    X_hgbc = hgbc.fit(X_train, y_train)
    y_train_pred = X_hgbc.predict(X_train)
    y_test_pred = X_hgbc.predict(X_test)

    hgbc_train= accuracy_score(y_train, y_train_pred)
    gbrt_test= accuracy_score(y_test, y_test_pred)
    print("그래디언트의 훈련 정확도/테스트 정확도 %.3f/%.3f" %(hgbc_train, gbrt_test))
    ...
    for idx, clf, tt in zip([0, 1], [tree, hgbc],['Decision tree', 'HistGradientBoosting']):
    ...
    plt.savefig(get_base_dir('images')+'/compare_histGradientBoosting_model_performance.png', dpi = 300)
    plt.show()
    plt.close()
