...
# from majerity_vote_classifier import MajorityVoteClassifier
...
def main():
    ...
    clf_labels = ['Logistic regression', 'Decision tree', 'KNN','Majority Vote']
    ...
    # mv_clf.set_params(verbose=True)
    # mv_clf.fit(X_train,y_train)
    # voting='soft' 일때 predict 메서드는 predict_proba 메소드에서 얻은 가장 큰 확률의
    # 클래스를 예측으로 삼습니다.
    # predict_proba 메서드는 각 분류기의 클래스 확률을 평균하여 반환합니다.
    # proba=mv_clf.predict_proba(X_test[:10])
    # print(proba)
    ...
    plt.savefig(get_base_dir('images')+'/evaluate_classifier_ROC_curve.png', dpi = 300)
    # plt.show()
    plt.close()
    ...
    plt.savefig(get_base_dir('images')+'/decision_regisons_MV.png', dpi = 300)
    plt.show()
    plt.close()
