...
if __name__ == '__main__':
    ...
    X_lgb = lgb.fit(X_train, y_train)
    y_train_pred = X_lgb.predict(X_train)
    y_test_pred = X_lgb.predict(X_test)

    lgb_train= accuracy_score(y_train, y_train_pred)
    lgb_test= accuracy_score(y_test, y_test_pred)
    print("LightGBM 훈련 정확도/테스트 정확도 %.3f/%.3f" %(lgb_train, lgb_test))

    # 결정트리와 LightGBM의 결정경계 비교
    ...
    for idx, clf, tt in zip([0, 1], [tree, lgb],['Decision tree', 'LightGBM']):
    ...
    plt.savefig(get_base_dir('images')+'/compare_LightGBM_model_performance.png', dpi = 300)
    plt.show()
    plt.close()
